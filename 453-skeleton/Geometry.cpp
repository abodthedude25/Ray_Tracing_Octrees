...
GPU_Geometry::GPU_Geometry()
	...
	// We'll bind texCoords to attribute location 3
	texCoordsBuffer(3, 2, GL_FLOAT)
{}
...
void GPU_Geometry::setTexCoords(const std::vector<glm::vec2>& texCoords) {
	texCoordsBuffer.uploadData(sizeof(glm::vec2) * texCoords.size(), texCoords.data(), GL_STATIC_DRAW);
}
